...
#import "MRCleanCaches.h"

@implementation MRCleanCaches
...
+ (BOOL)clearCachesFromDirectoryPath:(NSString *)dirPath{
    ...
    //获得全部文件数组
    NSArray *fileAry =  [MRCleanCaches getAllFileNames:dirPath];
    //遍历数组
    BOOL flag = NO;
    for (NSString *fileName in fileAry) {
        NSString *filePath = [dirPath stringByAppendingPathComponent:fileName];
        flag = [MRCleanCaches clearCachesWithFilePath:filePath];
